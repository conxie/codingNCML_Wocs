---
title: "Exercise_colortheory.rmd"
author: "Amy"
date: "9/12/25"
...
```{r}
plot_bar = penguins %>%
  ggplot(aes(y = flipper_length_mm, x = island ,
             fill = island)) +
...
plot_line = penguins %>%
  ggplot(aes(y = flipper_length_mm , x = body_mass_g , color = body_mass_g )) +
...
```{r}
plot_bar +
    scale_fill_manual(values = brewer.pal(n = 3, name = 'Pastel1'))

plot_jitter +
  scale_color_manual(values = c("pink", "pink2", "pink3"))
```
...
```{r}
plot_line +
  scale_color_gradient(low = "orchid", high = "purple4")
```
