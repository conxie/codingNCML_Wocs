...
penguins %>%
  ...
  geom_vline(aes(xintercept = 200), X = "black", size = 1)
...
Prediction:
--
I predict that Biscoe will have the highest average body mass, followed by Dream, then Torgersen because Biscoe includes Gentoo penguins, which are heavier.
--
...
Summary:
--
The plot confirms my preduction as Biscoe has the highest average body mass, while Dream and Torgersen are the lowest (relatively similar).
--
...
--
When using points, the same results is shown, but the differences feel less emphasized because the plot is more minimalist and precise but does not visually "shout" the differences as much as bars do.
--
...
```{r}
penguins %>%
  ggplot(aes(x = island, y = body_mass_g, group = 1)) + 
  stat_summary(fun = "mean", geom = "line", color = "blue", size = 1)
...
penguins %>%
  ggplot(aes(x = island, y = body_mass_g, group = 1)) + 
  stat_summary(fun = "mean", geom = "line", color = "blue", size = 1, linetype = "dashed")
```
...
penguins %>%
  ggplot(aes(x = island, y = body_mass_g, group = 1)) + 
  stat_summary(fun = "mean", geom = "line", color = "lightblue", size = 1, linetype = "dashed") +
...
penguins %>%
  ggplot(aes(x = island, y = body_mass_g, group = 1)) +
  stat_summary(fun = "mean", geom = "point", size = 3, color = "blue") +
  stat_summary(fun.data = "mean_se", geom = "linerange", width = 0.2, color = "blue") 
```
...
penguins %>%
  ggplot(aes(x = island, y = body_mass_g, group = 1)) +
  stat_summary(fun.data = "mean_se", geom = "pointrange", width = 0.2, color = "blue") 
```
...
penguins %>%
  ggplot(aes(y = body_mass_g, x = island)) +
  geom_jitter(width = 0.2, height = 0, alpha = 0.5, color = "darkgrey") +
  stat_summary(fun.min = mean, fun.max = mean, fun = mean, 
               geom = "crossbar", width = 0.5, color = "blue", size = 1)
```
...
library(ggridges)
penguins %>%
  ggplot(aes(x = body_mass_g, y = island, fill = island)) +
...
```{r}
mtcars2 %>%
  ggplot(aes(y = wt, x = gear, group = cyl, color = factor(cyl))) +
