...
load("C:/Users/prish/Downloads/codingNCML_Wocs-Prisha/codingNCML_Wocs-Prisha/Prisha/R Bootcamp/ggplot expanded - 2/Geoms and Different Graphs/diamonds_sample.RData") 
```
...
diamonds_sample %>%
  ggplot(aes(x = carat)) + 
  geom_histogram(color = "turquoise", fill = "cyan") + geom_vline(aes(xintercept = 0.7), color = "red", size=1)
```
...
Question:
  *What is the relationship between carat and depth?* 

Prediction:
  *An increase in carat will be correlated to an increase in depth*
...
Summary:
  *There is a weak positive correlation between depth and carat of the diamonds since the trendline has a slightly positive slope.*
...
```
Summary:
  *The greatest width of diamonds is between 5-7.5 since the count has the lightest blue color here. The greatest table values occur between 55 and 60.*
...
diamonds_sample %>%
  ...
  geom_point(color = "purple",
             size = 1,
             alpha = .6) + 
  geom_smooth(method = "lm", color = "red", se = FALSE) + geom_rug(size = 20,
           color = "blue",
           alpha = 0.1)
```
```{r}
...
(diamonds_sample %>%
  ...
  geom_bin_2d() + 
  geom_point(alpha = 0)) %>%
  ggExtra::ggMarginal(type = "histogram", color = "grey", fill = "blue", size = 10)
```
```

Do you think one of these two is better/more effective than the other? If so, which and why? If not, why not?

I think that the first one using geom_rug is better since it offers a visualization that is more pleasing to the eye and also demonstrates where there are additional data points compared to the rest of the dataset.
